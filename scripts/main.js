...
const translate = () => {
	for (let transalateWord of transalateWords) {
		transalateWord.innerHTML = arrLang[gameSettings.language][transalateWord.dataset.key];
	};
};

const getVolume = () => {
	eatSound.volume = backgroundSound.volume = gameWinSound.volume = gameOverSound.volume = gameSettings.volume;
};
...
const animationVolumeInp = () => {
	volumeInput.style.backgroundSize = (volumeInput.value - volumeInput.min) * 100 / (volumeInput.max - volumeInput.min) + '% 100%';
...
const drawSnake = () => {
	...
	snake.tails.forEach((item, index) => {
		index == 0 ? context.fillStyle = '#26B08F' : context.fillStyle = '#27574C';
...
langInp.addEventListener('change', () => {
	langInp.checked ? gameSettings.language = 'ua' : gameSettings.language = 'en';
	translate();
